            ...
            try:
                ...
                for uploaded_file in uploaded_files:
                    ...
                    try:
                        ...
                        # Coba metode 1: Baca sebagai HTML (Mayoritas kasus)
                        try:
                            dfs = pd.read_html(io.StringIO(html_content), flavor=['lxml', 'bs4', 'html5lib'])
                            if len(dfs) > 0:
                                df_temp = dfs[0]
                                if "No" in str(df_temp.iloc[0, 0]) or "Nama Fasyankes" in str(df_temp.iloc[0, 1]):
                                    df_temp.columns = df_temp.iloc[0]
                                    df_temp = df_temp[1:].reset_index(drop=True)
                        except ValueError:
                            # Jika "No tables found", Coba metode 2: Baca sebagai Excel asli
                            uploaded_file.seek(0)
                            df_temp = pd.read_excel(uploaded_file)
                            
                        # Simpan jika berhasil terekstrak dan tidak kosong
                        if df_temp is not None and not df_temp.empty:
                            data_frames.append(df_temp)
                        else:
                            st.warning(f"⚠️ Berkas '{uploaded_file.name}' dilewati karena kosong atau tidak memiliki tabel yang valid.")
                            
                    except Exception as e:
                        st.warning(f"⚠️ Gagal membaca berkas '{uploaded_file.name}'. File mungkin rusak. (Error: {e})")

                if not data_frames:
                    st.error("❌ Tidak ada satupun data valid yang berhasil diekstrak dari seluruh file laporan yang diunggah.")
                    st.stop()
                ...
                st.markdown("---")
                st.download_button(
                    label="📥 Unduh File Excel Profesional",
                    data=output_buffer,
